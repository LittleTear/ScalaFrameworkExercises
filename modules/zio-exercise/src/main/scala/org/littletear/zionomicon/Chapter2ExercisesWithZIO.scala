    ...
    object Cat extends ZIOAppDefault {
      ...
      def cat(files:Chunk[String]) =
        ZIO.foreach(files) {file =>
          readFilZio(file).flatMap(printLine)
        }
...
//      }
  ...
  object Exercise15{
    ...
    def saveUserRecordZio(user: User): ZIO[Any, Throwable, Unit] =
      ZIO.async{cb =>
        saveUserRecord(user,
          () => cb(ZIO.succeed(())),
          onFailure => cb(ZIO.fail(onFailure)))
      }
  }
  ...
  object Exercise17{
    import zio. { ZIOAppDefault => ZIOApp }
    ...
    object NumberGuessing extends ZIOAppDefault {
      val run =
        for{
          ...
          _ <- if(int.toString == num) Console.printLine("You guessed right !")
  ...
  object Exercise19{
    ...
    def readUntil(
      ...
      Console.readLine.flatMap{line =>
        if (acceptInput(line)) ZIO.succeed(line)
